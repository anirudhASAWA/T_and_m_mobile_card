...
function toggleTimer(processIndex) {
  ...
  if (process.subprocesses && process.subprocesses.length > 0) {
    process.subprocesses.forEach((subprocess, subprocessIndex) => {
      ...
      let activityTypeElement, remarksElement, personCountElement, productionQtyElement;
  ...
  // Re-render the interface - ensure form values are preserved
  renderInterface();
...
function recordLap(processIndex, subprocessIndex) {
  ...
  if (!activityTypeElement || !remarksElement || !personCountElement || !productionQtyElement) {
  ...
  const reading = {
    ...
    productionQty: productionQty,          // NEW: Include production quantity
...
function exportToExcel() {
  ...
  state.processes.forEach(process => {
    if (process.readings && process.readings.length > 0) {
      process.readings.forEach(reading => {
        // Add reading to export data, including production quantity but excluding time and timestamp
        allReadings.push({
          "Process": process.name,
  ...
  if (isMobile) {
    ...
    // For desktop, proceed with standard confirmation
    proceedWithExport('time_motion_study');
  }
}
...
function proceedWithExport(filename) {
  ...
  // After successful export, clear the readings data
  clearAllReadingsData();
  
  // Verify data was cleared by checking if any processes still have readings
  let stillHasReadings = false;
  state.processes.forEach(process => {
    if (process.readings && process.readings.length > 0) {
      stillHasReadings = true;
    }
  });
  
  if (stillHasReadings) {
    console.error("Failed to clear readings data!");
    // Try clearing again
    clearAllReadingsData();
  }
  
  // Force a re-render to make sure the UI reflects the cleared data
  renderInterface();
  
  // Show confirmation
  alert('Data has been exported to Excel and removed from the app.');
}
...
function clearAllReadingsData() {
  state.processes.forEach(process => {
    ...
    process.subprocesses.forEach(subprocess => {
      ...
      // Also reset other inputs to ensure a clean slate
  ...
  // Force re-render of the entire interface
  renderInterface();
  
  // If we're showing the recorded times container, hide it
  if (recordedTimesContainer) {
    recordedTimesContainer.style.display = 'none';
  }
  
  // Clear any mobile view recorded times
  if (isMobile) {
    const mobileView = document.getElementById('mobileView');
    if (mobileView) {
      // Re-render the mobile view to ensure all recordings are cleared
      renderMobileView();
    }
  }
  ...
  // Double-check that localStorage is updated
  try {
    ...
    console.error('Error saving cleared data to localStorage:', e);
  }
}
